...
TESSERACT_CFLAGS := $(shell $(PKG_CONFIG) --cflags tesseract 2>/dev/null)
TESSERACT_LIBS   := $(shell $(PKG_CONFIG) --libs tesseract 2>/dev/null)
LEPT_CFLAGS := $(shell $(PKG_CONFIG) --cflags lept 2>/dev/null)
LEPT_LIBS   := $(shell $(PKG_CONFIG) --libs lept 2>/dev/null)
...
# Combined flags used across targets
CPPFLAGS += $(MAGIC_CFLAGS) $(EXIV2_CFLAGS) $(TEXTCAT_CFLAGS) $(TESSERACT_CFLAGS) $(LEPT_CFLAGS) $(SQLITE3_CFLAGS) -DAPP_VERSION=\"$(VERSION)\"
LDLIBS += $(MAGIC_LIBS) $(EXIV2_LIBS) $(TEXTCAT_LIBS) $(TESSERACT_LIBS) $(LEPT_LIBS) $(SQLITE3_LIBS) -pthread
...
$(GTK_BIN): src/gui/main.cpp src/gui/clisperGTK.cpp
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(CPPFLAGS) $(GTK_CFLAGS) $^ -o $@ $(GTK_LIBS) $(SQLITE3_LIBS)
